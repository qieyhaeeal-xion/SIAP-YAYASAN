...
export type RoleCategory = 'admin' | 'guru' | 'wali';
...
export const LoginModal: React.FC<LoginModalProps> = ({ 
  ...
  const [selectedRole, setSelectedRole] = useState<UserRole>('admin_sistem');
  ...
  useEffect(() => {
    if (isOpen) {
      setActiveTab(initialRoleCategory);
      if (initialRoleCategory === 'admin') {
        setSelectedRole('admin_sistem');
        setUsername('admin');
      } else if (initialRoleCategory === 'guru') {
        setSelectedRole('guru');
        setUsername('guru_halim');
      } else if (initialRoleCategory === 'wali') {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Category Tabs: Admin | Guru | Wali */}
        <div className="bg-sky-50/80 border-b border-sky-100 p-2 grid grid-cols-3 gap-1 shrink-0">
          <button
            type="button"
            onClick={() => handleTabSelect('admin')}
            className={`py-2.5 px-2 rounded-xl text-xs font-extrabold flex flex-col sm:flex-row items-center justify-center gap-1.5 transition-all ${
              activeTab === 'admin'
        ...
        <form onSubmit={handleLoginSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="bg-sky-50 border border-sky-200 rounded-xl p-3.5 text-xs text-[#1A5276]">
            <p className="font-extrabold mb-1.5 flex items-center gap-1 text-[#1A5276]">
              ...
              {activeTab === 'admin' && 'Pilih Akun Demo Admin & Pengelola:'}
              {activeTab === 'guru' && 'Pilih Akun Demo Guru & Pengasuh:'}
              {activeTab === 'wali' && 'Pilih Akun Demo Orang Tua Murid / Wali:'}
            </p>
            ...
            {activeTab === 'admin' && (
              <div className="grid grid-cols-2 gap-1.5 mt-2">
                <button
                  type="button"
                  onClick={() => handlePresetSelect('admin_sistem', 'admin')}
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'admin_sistem' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  👑 Admin Sistem (Full)
                </button>
                <button
                  type="button"
                  onClick={() => handlePresetSelect('bendahara', 'bendahara')}
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'bendahara' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  💰 Bendahara Syahriyah
                </button>
                <button
                  type="button"
                  onClick={() => handlePresetSelect('admin_pesantren', 'admin_pesantren')}
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'admin_pesantren' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  🕌 Admin Pesantren
                </button>
                <button
                  type="button"
                  onClick={() => handlePresetSelect('admin_sekolah', 'admin_sekolah')}
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'admin_sekolah' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  🏫 Admin Sekolah (MTs/MA/SMK)
                </button>
              </div>
            )}
            ...
            {activeTab === 'guru' && (
              <div className="grid grid-cols-2 gap-1.5 mt-2">
                <button
                  ...
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'guru' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  📚 Ust. Abdul Halim (Guru Madin/Tahfidz)
                </button>
                <button
                  type="button"
                  onClick={() => handlePresetSelect('admin_kepengasuhan', 'admin_pengasuhan')}
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'admin_kepengasuhan' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  🏥 Ustadzah Nurul (Pengasuhan & UKS)
                </button>
              </div>
            )}
            ...
            {activeTab === 'wali' && (
              <div className="grid grid-cols-1 gap-1.5 mt-2">
                <button
                  ...
                  className={`p-2 rounded-lg text-[11px] font-bold text-left transition-all ${
                    selectedRole === 'wali_santri' ? 'bg-[#1A5276] text-white shadow' : 'bg-white text-gray-700 hover:bg-sky-100 border border-gray-200'
                  }`}
                >
                  👨‍👩‍👧 H. Abdullah Mahmud (Wali Santri Ahmad Farhan)
                </button>
              </div>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">
              {activeTab === 'admin' && 'Username / NIP Admin'}
              {activeTab === 'guru' && 'NIP / ID Guru / Ustadz'}
              {activeTab === 'wali' && 'Nomor Induk Santri (NIS) / HP Ortu'}
            </label>
