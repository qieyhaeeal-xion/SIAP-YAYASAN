  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-extrabold text-[#1A5276] flex items-center gap-2">
            <School className="w-5 h-5 text-[#1ABC9C]" />
            Sub Sekolah Formal: MTs, MA, SMK
          </h2>
          <p className="text-xs text-[#566573]">Pengelolaan master unit sekolah formal, jurusan, dan kelas formal</p>
        </div>
      ...
      {activeTabSub === 'sekolah' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Daftar Unit Sekolah Formal</h3>
            <button
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Kode Sekolah</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {unitSekolahList.map(s => (
                  <tr key={s.id} className="hover:bg-sky-50 transition-colors">
                    <td className="p-3 font-bold text-[#1A5276]">{s.kodeSekolah}</td>
                    <td className="p-3 font-semibold">{s.namaSekolah}</td>
                    <td className="p-3 text-gray-700">{s.kepalaSekolah}</td>
      ...
      {activeTabSub === 'jurusan' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Daftar Jurusan Sekolah</h3>
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Kode Jurusan</th>
                  <th className="p-3">Nama Jurusan</th>
                  <th className="p-3">Induk Sekolah</th>
                  <th className="p-3 rounded-tr-lg text-center">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {jurusanList.map(j => {
                  ...
                  return (
                    <tr key={j.id} className="hover:bg-sky-50 transition-colors">
                      <td className="p-3 font-bold text-[#1A5276]">{j.kodeJurusan}</td>
                      <td className="p-3 font-semibold">{j.namaJurusan}</td>
                      <td className="p-3 text-gray-700">{s?.namaSekolah || '-'}</td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => {
      ...
      {activeTabSub === 'kelas' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Daftar Kelas Formal</h3>
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Kode Kelas</th>
                ...
                {kelasSekolahList.map(k => {
                  ...
                  return (
                    <tr key={k.id} className="hover:bg-sky-50 transition-colors">
                      <td className="p-3 font-bold text-[#1A5276]">{k.kodeKelas}</td>
                      <td className="p-3 font-semibold">{k.namaKelas}</td>
                      <td className="p-3 text-gray-700">{s?.namaSekolah || '-'}</td>
