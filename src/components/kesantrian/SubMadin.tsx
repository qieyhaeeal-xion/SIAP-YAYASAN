    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-[#566573]">Master Marhalah (Ula, Wustho, Ulya), Kelas Diniyah, dan Kitab Hafalan</p>
        </div>
      ...
      <div className="bg-sky-50 border border-sky-200 p-3.5 rounded-xl flex flex-col sm:flex-row items-center gap-3">
        <div className="flex items-center gap-2 text-xs font-bold text-[#1A5276] shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-md">
          ...
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase">1. PILIH MARHALAH</label>
            <select
              ...
              className="w-full px-2.5 py-1.5 bg-white border border-gray-300 rounded-lg text-xs font-semibold focus:ring-2 focus:ring-[#1ABC9C]"
            >
          ...
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase">2. PILIH KELAS MADIN</label>
            <select
      ...
      {activeTabSub === 'marhalah' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Master Marhalah Diniyah</h3>
            <button
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Kode Marhalah</th>
                  <th className="p-3">Nama Marhalah</th>
                  <th className="p-3">Tingkat Jenjang</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {marhalahList.map(m => (
                  <tr key={m.id} className="hover:bg-sky-50 transition-colors">
                    <td className="p-3 font-bold text-[#1A5276]">{m.kodeMarhalah}</td>
                    <td className="p-3 font-semibold">{m.namaMarhalah}</td>
                    <td className="p-3 font-bold text-[#1ABC9C]">Jenjang ke-{m.tingkat}</td>
      ...
      {activeTabSub === 'kelas' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Daftar Kelas Madin</h3>
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Nama Kelas Madin</th>
                  <th className="p-3">Marhalah</th>
                  <th className="p-3">Wali Kelas Diniyah</th>
                  <th className="p-3 rounded-tr-lg text-center">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {filteredKelasList.map(k => {
                  ...
                  return (
                    <tr key={k.id} className="hover:bg-sky-50 transition-colors">
                      <td className="p-3 font-bold text-[#1A5276]">{k.namaKelas}</td>
                      <td className="p-3 text-gray-700 font-semibold">{m?.namaMarhalah || '-'}</td>
                      <td className="p-3 text-gray-600">{k.waliKelas}</td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => {
      ...
      {activeTabSub === 'kitab' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-sm text-[#1A5276]">Daftar Kitab Hafalan (Aqidatul Awam, Imriti, Alfiyah)</h3>
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#1A5276] text-white font-bold uppercase tracking-wider">
                  <th className="p-3 rounded-tl-lg">Nama Kitab</th>
                  <th className="p-3">Marhalah Madin</th>
                  <th className="p-3">Total Bait</th>
                  <th className="p-3">Pengampu Kitab</th>
                  <th className="p-3 rounded-tr-lg text-center">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {kitabList.map(kt => {
                  ...
                  return (
                    <tr key={kt.id} className="hover:bg-sky-50 transition-colors">
                      <td className="p-3 font-bold text-[#1A5276] flex items-center gap-2">
                        <BookMarked className="w-4 h-4 text-[#1ABC9C]" />
                        {kt.namaKitab}
                      </td>
                      <td className="p-3 text-gray-700 font-semibold">{m?.namaMarhalah || '-'}</td>
                      <td className="p-3 font-bold text-emerald-700">{kt.totalBait} Bait</td>
                      <td className="p-3 text-gray-600">{kt.pengampu}</td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => {
